...
toFrag0 (Pair x v) f@(G.PropEq (Var x') v') | x == x' && v == v' = Just F0.Atom
toFrag0 _ f@(G.PropEq {}) = Nothing

-- | Evaluate `Frag0` to `Frag1`
toFrag1 :: Frag0 -> Frag1
toFrag1 F0.Atom          = F1.Atom
...
toFrag2 :: Frag1 -> Frag2
toFrag2 F1.Atom      = F2.Atom
toFrag2 F1.NotAtom   = F2.NotAtom
...
toGuardedFormula (Pair x v) F2.Atom    = G.PropEq (Var x) v
toGuardedFormula (Pair x v) F2.NotAtom = G.Not (G.PropEq (Var x) v)
...
findAtoms (G.PropEq (Var x) b) set = Set.insert (Pair x b) set
findAtoms (G.PropEq a b) set       = set
